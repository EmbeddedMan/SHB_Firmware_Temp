...
# 2 "<built-in>" 2
# 1 "../demo_src/usb_device.c" 2
# 21 "../demo_src/usb_device.c"
# 1 "../demo_src/usb.h" 1
# 30 "../demo_src/usb.h"
...
void LoadBDTandSetUOWN(unsigned char BDTIndexToLoad);
# 49 "../demo_src/usb_device.c"
unsigned char bTRNIFCount;
unsigned char ctrl_trf_state;
unsigned char ctrl_trf_session_owner;
POINTER pSrc;
POINTER pDst;
WORD_VAL wCount;
unsigned char short_pkt_status;
CTRL_TRF_SETUP SetupPkt;
_Bool EP0OutOddNeedsArmingNext;
BDT TempBDT;
unsigned char usb_device_state;
USB_DEVICE_STATUS usb_stat;
unsigned char usb_active_cfg;
unsigned char usb_alt_intf[1];



#pragma warning disable 1090

unsigned char USTATSave;
unsigned int Counter;
_Bool DeviceIsSoftDetached;
# 139 "../demo_src/usb_device.c"
volatile BDT ep0BoEven @0x400;
volatile BDT ep0BoOdd @0x400 +4;
volatile BDT ep0Bi @0x400 +8;



volatile BDT ep1Bo @0x400 +12;
volatile BDT ep1Bi @0x400 +16;
# 223 "../demo_src/usb_device.c"
volatile unsigned char EP0OutEvenBuf[8] @(0x400 + 12 + (1 * 8));
volatile unsigned char EP0OutOddBuf[8] @(0x400 + 12 + (1 * 8))+8;
volatile CTRL_TRF_DATA CtrlTrfData @(0x400 + 12 + (1 * 8))+8 +8;
...
volatile unsigned char hid_report_out[64] @(0x400 + 12 + (1 * 8))+8 +8 +8;
volatile unsigned char hid_report_in[64] @(0x400 + 12 + (1 * 8))+8 +8 +8 +64;
# 295 "../demo_src/usb_device.c"
void USBDeviceInit(void)
{
...
void USBSoftAttach(void)
{
    ...
    if(DeviceIsSoftDetached == 1)
    {
        USBDisableWithLongDelay();
    }

    UCON = 0;
    UCFG = 0x10|0x00|0x04|0x01;
    UIE = 0;
    UCONbits.USBEN = 1;
...
void USBSoftDetach(void)
{
    ...
    UCON = 0x00;
    usb_device_state = 0;
    DeviceIsSoftDetached = 1;
}
# 375 "../demo_src/usb_device.c"
void USBCheckBusStatus(void)
{
...
# 409 "../demo_src/usb_device.c"
        if(UCONbits.USBEN == 0)
            USBSoftAttach();

}
# 441 "../demo_src/usb_device.c"
void USBDeviceTasks(void)
{
    ...
    for(bTRNIFCount = 0; bTRNIFCount < 4; bTRNIFCount++)
    {
        if(UIRbits.TRNIF)
        {
            ...
            if((USTAT & 0x7C) == ((0x00<<3)|(0<<2)))
            {
                ...
                if(pBDTEntry->Stat.PID == 0b00001101)
                {
...
# 587 "../demo_src/usb_device.c"
void USBSuspend(void)
{
    static unsigned char UIESave;
# 624 "../demo_src/usb_device.c"
    UIESave = UIE;
    UIE = 0b00000100;
...
void USBProtocolResetHandler(void)
{
    ...
    UIE = 0b01111011;
    UADDR = 0x00;
    UEP1=0x00;UEP2=0x00;UEP3=0x00; UEP4=0x00;UEP5=0x00;UEP6=0x00;UEP7=0x00;;
    UEP0 = 0x06|0x10;
    UCONbits.PPBRST = 1;
    ...
    TempBDT.Stat._byte = 0x00|0x04;
    LoadBDTandSetUOWN(0);
    EP0OutOddNeedsArmingNext = 1;
    usb_stat._byte = 0x00;
    usb_active_cfg = 0;
    USBCBInitEP(0);

}
# 790 "../demo_src/usb_device.c"
void USBCtrlTrfSetupHandler(void)
{
    ...
    {
        ep0BoEven.Stat._byte = 0x00;
        EP0OutOddNeedsArmingNext = 0;
    }
    if(ep0BoOdd.Stat.UOWN == 1)
    {
        ep0BoOdd.Stat._byte = 0x00;
        EP0OutOddNeedsArmingNext = 1;
    }
    ctrl_trf_state = 0;
...
# 993 "../demo_src/usb_device.c"
void USBCtrlTrfInHandler(void)
{



    if(usb_device_state == 4)
    {
    ...
    if(ctrl_trf_state == 1)
    {
        ...
        if(short_pkt_status == 2)
        {


            ep0Bi.Stat._byte = 0x04;
            ep0Bi.Stat._byte |= 0x80;
        }
        else
        {

            if(ep0Bi.Stat.DTS == 0)
                ep0Bi.Stat._byte = 0x40|0x08;
            else
                ep0Bi.Stat._byte = 0x00|0x08;
...
void USBCtrlTrfTxService(void)
{
    ...
    if(wCount.Val < 8)
    {
        ...
        if(short_pkt_status == 0)
        {
            short_pkt_status = 1;
        }
        else if(short_pkt_status == 1)
        {
            short_pkt_status = 2;
        }
    }
    ...
    pDst.bRam = (unsigned char*)&CtrlTrfData;
    if(usb_stat.ctrl_trf_mem == 1)
    {
        while(bytes_to_send)
        {
            *pDst.bRam = *pSrc.bRom;
...
void USBCtrlEPServiceComplete(void)
{
    ...
    if(ctrl_trf_session_owner == 0)
    {
        ...
        ep0Bi.Stat._byte = 0x04;
        ep0Bi.Stat._byte |= 0x80;
        TempBDT.Stat._byte = 0x04;
        if(EP0OutOddNeedsArmingNext == 1)
        {
            LoadBDTandSetUOWN(1);
            EP0OutOddNeedsArmingNext = 0;
        }
    ...
    {
        if(SetupPkt.DataDir == 1)
        {
# 1197 "../demo_src/usb_device.c"
            ctrl_trf_state = 1;
            ...
            ep0Bi.ADR = (unsigned char*)&CtrlTrfData;
            ep0Bi.Stat._byte = 0x40|0x08;
            ep0Bi.Stat._byte |= 0x80;
        }
        else
        {
# 1234 "../demo_src/usb_device.c"
            ctrl_trf_state = 2;
...
# 1294 "../demo_src/usb_device.c"
void USBCheckStdRequest(void)
{
    if(SetupPkt.RequestType != 0x00) return;
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case 8:
            ctrl_trf_session_owner = 1;
            pSrc.bRam = (unsigned char*)&usb_active_cfg;
        ...
        case 10:
            ctrl_trf_session_owner = 1;
            pSrc.bRam = (unsigned char*)&usb_alt_intf+SetupPkt.bIntfID;
            usb_stat.ctrl_trf_mem = 0;
            wCount.v[0] = 1;
            break;
        case 11:
...
void USBStdGetDscHandler(void)
{
    if(SetupPkt.bmRequestType == 0x80)
    {
        switch(SetupPkt.bDscType)
        {
            case 0x01:
                ctrl_trf_session_owner = 1;
                pSrc.bRom = (const unsigned char*)&device_dsc;
...
void USBStdGetStatusHandler(void)
{
    ...
    switch(SetupPkt.Recipient)
    {
        ...
        case 2:
            ...
            pDst.bRam = (unsigned char*)&ep0BoEven+(SetupPkt.EPNum*8)+(SetupPkt.EPDir*4)+4;
            if(*pDst.bRam & 0x04)
...
# 1516 "../demo_src/usb_device.c"
void USBStdFeatureReqHandler(void)
{

    if((SetupPkt.bFeature == 0x01)&&(SetupPkt.Recipient == 0))
    {
        ctrl_trf_session_owner = 1;
    ...
    if((SetupPkt.bFeature == 0x00)&&(SetupPkt.Recipient == 2)&&(SetupPkt.EPNum != 0))
    {
        ctrl_trf_session_owner = 1;
...
void LoadBDTandSetUOWN(unsigned char BDTIndexToLoad)
{
    ...
    TempBDT.Cnt = 8;
    TempBDT.ADR = (unsigned char*)&EP0OutOddBuf[0];
    if(BDTIndexToLoad == 0)
    {
        TempBDT.ADR = (unsigned char*)&EP0OutEvenBuf[0];
        pBDTEntry = (volatile BDT*)0x400;
    }
    else
    {
        pBDTEntry = (volatile BDT*)(0x400 + 4);
    }
